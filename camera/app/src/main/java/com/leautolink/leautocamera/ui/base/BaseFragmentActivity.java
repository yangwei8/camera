...
public abstract class BaseFragmentActivity extends FragmentActivity implements IToastSafe {
    ...
    public void onEventMainThread(CameraDisconnectEvent event) {
        ...
        if (this.getClass().getSimpleName().equals("HomeActivity_")) {
            return;
        } else {
        ...
        if (isCurrentActivity()) {
            showConfirmDialog(getString(R.string.other_phone_connected), R.drawable.dialog_yes, new OnDialogListener() {
                ...
                public void onCancel() {
                    WifiAdminV2 adminV2 = new WifiAdminV2(BaseFragmentActivity.this);
                    adminV2.forget(SpUtils.getInstance(BaseFragmentActivity.this).getStringValue(Constant.WIFI_SSID));
                    HomeActivity_.intent(BaseFragmentActivity.this).start();
                }
            });
        }
    }
    public void onEventMainThread(UDPTimeOutEvent event) {
        Logger.e("BaseFragmentActivity ========>  UDPTimeOutEvent   :" + this.getClass().getSimpleName());
    ...
    public void showConfirmDialog(final String text, final OnDialogListener listener) {
        if (tid == Process.myTid()) {
            if (alertDialog == null) {
//                alertDialog = new MaterialDialog(BaseFragmentActivity.this);
...
//                    }
//                }).show();
                NormalDialog.Builder builder = new NormalDialog.Builder(BaseFragmentActivity.this).setMessage(text).setPositiveButton(R.drawable.dialog_yes, new DialogInterface.OnClickListener() {
                    @Override
...
//                        }).show();
    ...
    public void showConfirmDialog(final String text, final int btnDrableId, final OnDialogListener listener) {
        if (tid == Process.myTid()) {
//            if (alertDialog == null) {
//                alertDialog = new MaterialDialog(BaseFragmentActivity.this).setMessage(text).setNegativeButton(btntext, new View.OnClickListener() {
//                    @Override
//                    public void onClick(View v) {
//                        dismissDialog();
//                        if (listener != null) {
...
//            }
    ...
    protected void showLoading() {
        showLoading(getResources().getString(R.string.base_activity_loading_text));
    }

    public void showLoading(final String tip) {
        if (tid == Process.myTid()) {
            if (isValidContext(BaseFragmentActivity.this)) {
            ...
            post(new Runnable() {
                ...
                public void run() {
                    if (isValidContext(BaseFragmentActivity.this)) {
                        if (loadingViewDialog == null) {
                            loadingViewDialog = LoadingDiglog.createLoadingDialog(BaseFragmentActivity.this, tip);
                            loadingViewDialog.setCanceledOnTouchOutside(false);
                        }
                        loadingViewDialog.show();
                    }
